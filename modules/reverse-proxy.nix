...
in {
  ...
  config = mkIf cfg.enable {
    services.nginx = {
      ...
      virtualHosts = let
        sslConfig = if cfg.ssl.enable then {
          forceSSL = true;
          sslCertificate = if cfg.ssl.useSelfSigned
            then "/var/lib/acme/${cfg.domain}/cert.pem"
            else cfg.ssl.certificatePath;
          sslCertificateKey = if cfg.ssl.useSelfSigned
            then "/var/lib/acme/${cfg.domain}/key.pem"
            else cfg.ssl.keyPath;
        } else {};
      ...
      in {
        "${cfg.cockpit.subdomain}.${cfg.domain}" = mkIf cfg.cockpit.enable (sslConfig // authentikConfig // {
          locations."/" = {
        ...
        "${cfg.filebrowser.subdomain}.${cfg.domain}" = mkIf cfg.filebrowser.enable (sslConfig // authentikConfig // {
          locations."/" = {
